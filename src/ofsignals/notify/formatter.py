...
STATUS_TEXT = {
    "filled": ("🎯", "entry filled"),
    "tp1": ("✅", "TP1 hit — 40% closed, stop to break-even"),
    "tp2": ("✅", "TP2 hit — 35% closed, trailing the runner"),
    "tp3": ("🏁", "TP3 hit — runner closed"),
    "stopped": ("❌", "stopped out"),
    "closed_be": ("⚪", "closed at break-even"),
    "expired": ("⌛", "expired unfilled — not chased"),
...
def format_update(signal: OpenSignal, new_status: str, price: float,
    ...
    icon, text = STATUS_TEXT.get(new_status, ("ℹ️", new_status))
    side = "LONG" if signal.direction is Direction.LONG else "SHORT"
    parts = [
        f"{icon} <b>{html.escape(signal.symbol)}</b> {side} · {signal.mode}",
        f"{text} @ <code>{fmt_price(price)}</code>",
    ]
    if r_multiple is not None:
        parts.append(f"Result: <b>{r_multiple:+.2f}R</b>")

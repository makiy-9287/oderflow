        ...
        await self._db.execute(
            """INSERT OR IGNORE INTO signals
               (signal_id, generated_at, symbol, direction, mode, entry_high, entry_low,
                stop_loss, tp1, tp2, tp3, rr2, leverage, sl_distance_pct, score,
                score_breakdown, rationale, tags, price_at_signal, valid_until, status)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                ...
                signal.rr["tp2"], int(signal.leverage["recommended"]),
